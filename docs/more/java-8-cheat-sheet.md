...
* _immutable_ (produce new streams)
* _lazy_ (only computes what is necessary !)
...
**sorted** Sort elements (must be _Comparable_)
...
Use _mapToX_ (mapToObj, mapToDouble, etc.) if the function yields Object, double, etc. values.
...
_PS_: Don't forget Optional (like `Map<T, Optional<T>>`) with some Collection methods (like `Collectors.maxBy`).
...
_PS_: Work with the streams library. Eg. use `filter(x -> x.length() < 9)` instead of a `forEach` with an `if`.
...
Type _must_ be given explicitly.
