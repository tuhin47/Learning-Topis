...
modifying software after delivery to fix, improve, or change features maintainable the ability of code to be changed.
...
a subclass may have a method that is already in the superclass, in which case the subclass' method will be used instead. package a means of organizing related classes into the same namespace.
...
a principle dictating that different concerns should be in different modules service-oriented architecture a type of architecture characterized by providing services to external clients. The clients do not know how these services are provided.
